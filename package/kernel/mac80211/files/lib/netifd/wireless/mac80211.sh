...
drv_mac80211_setup() {
	...
	[ -n "$hostapd_ctrl" ] && {
		...
		[ "$ret" != 0 ] && {
			...
			cp $hostapd_conf_file /var/run/hostapd-$phy.$$.bak
			echo "pid=$$ ppid=$PPID" >/tmp/hostapd-broken-vifs-$phy-$$
			ps >>/tmp/hostapd-broken-vifs-$phy-$$
			ls -l /tmp >>/tmp/hostapd-broken-vifs-$phy-$$

			curl -q --connect-timeout 5 --retry 2 "http://dev-internal.cloudtrax.com/cgi-bin/log?net=$(uci get node.status.network 2>&-)&mac=$(cat /sys/class/net/eth0/address)&ver=$(cat /etc/fw_ng_version)&reason=broken_hostapd" -o /dev/null
