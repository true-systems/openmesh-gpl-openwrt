wpa_supplicant_setup_vif() {
	...
	[ -z "$bridge" ] && {
		...
		[ "$vif" = "orphan" ] && brctl addbr "br-meship" && bridge="br-meship"
...
EOF
	if [ -n "$proto" -o "$key_mgmt" = "NONE" ]; then
		# for ADHOC: sleep 2 seconds to avoid launching wpa_s while
		# hostapd is still initialising beaconing an an AP vif
		[ "$mode" = "adhoc" ] && sleep 2
